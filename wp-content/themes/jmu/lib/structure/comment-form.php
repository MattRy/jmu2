...
function wsm_genesis_comment_form_args( $defaults ) {
	...
	$author = '<p class="comment-form-author">' .
			 '<input id="author" name="author" type="text" placeholder="' . __( 'Name', 'wsm' ) .   ( $req ? '*' : '' ) .'" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" tabindex="1"' . $aria_req . ' />' .
			 '<label for="author">' . __( 'Name', 'wsm' ) .   ( $req ? '<span class="required">*</span>' : '' ) .'</label> ' .
			 '</p><!-- .form-section-author .form-section -->';

	$email = '<p class="comment-form-email">' .
			'<input id="email" name="email" type="text" placeholder="' . __( 'Email', 'wsm' ) .   ( $req ? '*' : '' ) .'" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" tabindex="2"' . $aria_req . ' />' .
			'<label for="email">' . __( 'Email', 'wsm' ) . ( $req ? '<span class="required">*</span>' : '' ) . '</label> ' .
			'</p><!-- .form-section-email .form-section -->';

		$url = '<p class="comment-form-url">' .
			'<input id="url" name="url" type="text" placeholder="' . __( 'Website', 'wsm' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" tabindex="2" />' .
	         '<label for="url">' . __( 'Website', 'wsm' ) . '</label> ' .
	         '</p><!-- .form-section-url .form-section -->';

	$comment_field = '<p class="comment-form-comment">' .
					'<label for="comment">' . __( 'Comment', 'wsm' ) . '</label>' .
	                  '<textarea id="comment" name="comment" placeholder="' . __( 'Comment', 'wsm' ) . '" cols="45" rows="8" tabindex="4" aria-required="true"></textarea>' .
	                 '</p>';
